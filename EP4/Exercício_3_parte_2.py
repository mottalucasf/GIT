...
def solução_exata(x):
    return x/2 * (3*x**2 - 14*x + 15)


# Implementando o vetor d, para f(x)=2*pi*sin(pi*x), utilizando a função g, tal que g'' = f
def g(x):
    return (3*x**5)/20 - (7*x**4)/6 + x**3 + 7*x**2
...
# Leitura do programa
print('Vamos analisar o erro entre a solução exata e a solução aproximada do Exercício 3 - Parte 2.')
